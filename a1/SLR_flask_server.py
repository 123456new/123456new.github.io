...
if __name__ == '__main__':
    ...
    if not os.path.exists(model_save_path):
        # 尝试自动下载模型（若提供下载地址）
        model_download_url = server_config.get("model_download_url") or os.environ.get("MODEL_DOWNLOAD_URL")
        if model_download_url:
            logger.logger.info(f"Model not found at {model_save_path}. Attempting to download from {model_download_url}...")
            try:
                _download_file(model_download_url, model_save_path)
                logger.logger.info("Model downloaded successfully.")
            except Exception as e:
                logger.logger.error(f"Failed to download model: {e}")
                exit()
        else:
...
def _download_file(url, dest_path):
    """Download a file from url to dest_path using urllib"""
        ...
        with open(dest_path, 'wb') as out_file:
            ...
            while True:
                ...
                if total_size:
                    ...
                    logger.logger.info(f"Downloading model... {percent}%")
