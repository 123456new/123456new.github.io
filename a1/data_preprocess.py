...
for index in range(start_idx, end_idx):
    ...
    for one_mat in os.listdir(one_label_dir):
        ...
        if len(key_frames) == keyframe_num:
            one_label_data.append(key_frames)
    # 过滤掉样本不足的类别
    if len(one_label_data) < 10:
        print(f"Class {index} has only {len(one_label_data)} samples, skipped")
        continue
    one_label_data_array = np.array(one_label_data, dtype=np.float32)
    data.append(one_label_data_array)
    one_label = len(data) - 1
...
all_data = []
all_label = []
for class_data, class_label in zip(data, label):
    for sample in class_data:
        if sample.shape[0] == keyframe_num:
            all_data.append(sample[:, need_index])
            all_label.append(class_label[0])
data_array = np.array(all_data, dtype=np.float32)
label_array = np.array(all_label, dtype=np.int16).reshape(-1, 1)
# 转换为相对坐标
